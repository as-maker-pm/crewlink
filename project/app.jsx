...
const CompanyThreadsPanel = ({ role, onClose }) => {
  ...
  return (
    <div style={{width:360,flexShrink:0,borderLeft:'1px solid var(--border)',display:'flex',
      flexDirection:'column',background:'var(--card)',overflow:'hidden'}}>

      {/* Header */}
      <div style={{padding:'0 18px',height:46,display:'flex',alignItems:'center',gap:8,
        borderBottom:'1px solid var(--border)',flexShrink:0}}>
        <Icon name="message" size={15}/>
        <span style={{fontSize:14,fontWeight:700,flex:1}}>Discussions</span>
...
const CompanyThread = ({ companyId, role, users }) => {
  ...
  const MentionDrop = ({field}) => (showMentions && mentionFor===field && mentionMatches.length>0) ? (
    <div style={{position:'absolute',zIndex:60,top:'calc(100% + 4px)',left:0,minWidth:220,background:'var(--card)',
      ...
      <div style={{padding:'6px 12px',fontSize:10,fontWeight:700,textTransform:'uppercase',letterSpacing:'.06em',
        ...
        : <div style={{display:'flex',flexDirection:'column',gap:20}}>
            {comments.map(c=>(
              <div key={c.id}>
                ...
                {c.replies.length > 0 && (
                  <div style={{marginLeft:44,paddingLeft:12,borderLeft:'2px solid var(--border)',marginTop:10,display:'flex',flexDirection:'column',gap:12}}>
                    {c.replies.map(r=>(
                ...
                {replyingTo===c.id && (
                  <div style={{marginLeft:44,paddingLeft:12,borderLeft:'2px solid var(--border)',marginTop:10,display:'flex',gap:8}}>
                    <Av a={me} sz={26}/>
                    <div style={{flex:1}}>
                      <Input value={replyText} onChange={setReplyText} field="reply"
                        onSubmit={()=>addReply(c.id)} onCancel={()=>setReplyingTo(null)}
                        placeholder="Reply… @ to mention" submitLabel="Reply"/>
                    </div>
                  </div>
                )}
              </div>
  ...
  return (
    <div>
      ...
      {showDiscussions && canDiscuss
        ? <div style={{display:'flex',flexDirection:'row',border:'1px solid var(--border)',borderRadius:12,overflow:'hidden',background:'var(--card)'}}>
            {tableBlock}
